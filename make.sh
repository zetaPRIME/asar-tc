...
if [[ "$OSTYPE" == 'msys' ]]; then
    ...
    if [[ "$1" != "-q" ]]; then
        ...
        g++ -std=c++98 -U__STRICT_ANSI__ -Wall -Werror -Wno-unused-result -Wno-uninitialized *.cpp -oasar.dll -s -O3 -fno-rtti -DINTERFACE_LIB -Dstricmp=strcasecmp -DGNUWIN32 -Dwindows -Dfixgw -DRELEASE -fvisibility=hidden -fvisibility-inlines-hidden -shared
    fi
else
    echo \* Detected linux
